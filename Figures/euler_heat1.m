...
##plot exact solution
u=25.*exp(-k*t)+5
plot(t,u,'g-',linewidth=3,label='Exact Solution')
title(eqntex)
...
tvals=[t0]
vvals=[vi]
for ti in arange(t0,12*60*60+dt,dt):
  plot([ti,ti],[0,vi],'k.--')
  tvals.append(ti)
  vvals.append(vi)
  dv=-k*(vi-5.)*dt
  print ti/3600, '&', ti ,'& %.1f'%vi , '& %.1f'%dv, r'\\'
  vi=vi+dv
  
tvals.append(ti)
vvals.append(vi)
plot(tvals,vvals,'k.-',label=r'$\Delta t = %ds$'%dt)
legend()  
